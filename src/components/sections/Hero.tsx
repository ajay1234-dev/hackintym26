...
export function Hero() {
  ...
  return (
    <Section id="home" className="min-h-[90vh] flex items-center justify-center overflow-hidden">
      {/* Animated Background */}
      ...
      <div className="text-center z-10 w-full max-w-4xl mx-auto px-4">
        ...
        >
           {[1, 2, 3].map((num) => (
             <div key={num} className="w-20 h-20 md:w-24 md:h-24 rounded-full glass bg-white/5 flex items-center justify-center border border-hack-neonCyan/30 hover:border-hack-neonCyan shadow-[0_0_15px_rgba(74,222,128,0.2)] hover:shadow-[0_0_25px_rgba(74,222,128,0.6)] transition-all overflow-hidden relative group">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={`/clubs/club${num}.png`} alt={`Club ${num}`} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
             </div>
           ))}
        </motion.div>
          ...
          {[
            ...
            <div key={item.label} className="flex flex-col items-center">
              <div className="w-16 h-16 md:w-24 md:h-24 glass-card flex items-center justify-center rounded-xl md:rounded-2xl mb-2 border-white/20">
                <span className="text-2xl md:text-5xl font-bold font-mono text-white text-shadow-neon">{item.value.toString().padStart(2, '0')}</span>
              </div>
